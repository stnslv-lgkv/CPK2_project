...
bac_gr_funcs <- bac_gr_funcs %>% 
  mutate(Bacteria_tax = paste(TaxonName, Rank, sep = "_"),
         Bacteria_category_eng = case_when(
           Bacteria_category == "Условно-нормальный" ~ "Non-beneficial", 
           Bacteria_category == "Патоген" ~ "Pathogenic",
...
fb140_mrg <- fb140_long %>% 
  full_join(fb140_stats, by="patient_ID") %>% 
  mutate(source = "IBS")

fbh_mrg <- fbh_long %>% 
  full_join(fbh_stats, by="patient_ID") %>% 
...
fb_all <- fb_long %>% 
  left_join(bac_gr_funcs, by="Bacteria_tax")
...
NA_count_foo <- function(dt) {
  dt %>%
    ...
    mutate(across(everything()), round(. / n, 2)) %>% 
    select(-n)
}
